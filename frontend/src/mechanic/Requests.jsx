...
const Requests = () => {
  ...
  const getData = async () => {
    try {
      const requests = await axios.get(
        `http://54.163.203.92:5000/request/mechanic/${mechanic.name}`
      );
  ...
  const handleSubmit = async (id) => {
    try {
      console.log(id, "Id")
      const res = await axios.patch(`http://54.163.203.92:5000/request/accept/${id}`);
      const requests = await axios.get(
        `http://54.163.203.92:5000/request/mechanic/${mechanic.name}`
      );
      setData(requests.data);
  ...
  const submit = async (id) => {
    try {
      console.log(id, "Id")
      const res = await axios.patch(`http://54.163.203.92:5000/request/reject/${id}`);
      const requests = await axios.get(
        `http://54.163.203.92:5000/request/mechanic/${mechanic.name}`
      );
      setData(requests.data);
