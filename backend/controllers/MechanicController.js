...
export const updatePassword = async(req,res)=>{
  try{
      const data = req.body;
      console.log(data)
      const customer=await Mechanic.findOneAndUpdate({email:data.email},data,{new:true})
...
export const addMeachanic=async(req,res) => {
    try{
        const data=req.body;
        const cus = await Mechanic.find({ email: data.email });
        if(cus){
          res.status(400).send({ msg: "Mechanic already exists" });
        }else{
